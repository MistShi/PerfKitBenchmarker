...
EXPECTED_C_METADATA = {
    'bench_version': 'C small',
}

EXPECTED_JAVA_METADATA = {
    'bench_version': 'Java small',
    'java.vendor': 'Oracle Corporation',
    'os.version': '3.16.0-25-generic',
    'os.arch': 'amd64',
    'os.name': 'Linux',
...
class Scimark2BenchmarkTestCase(unittest.TestCase):
  ...
  def testParseResults(self):
    samples = []
    samples.extend(scimark2_benchmark.ParseResults(TEST_OUTPUT_C))
    samples.extend(scimark2_benchmark.ParseResults(TEST_OUTPUT_JAVA))
